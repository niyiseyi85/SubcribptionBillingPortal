...
public sealed class Subscription : AggregateRoot
{
    ...
    public static Subscription Create(Guid customerId, SubscriptionPlan plan)
    {
        if (plan is null)
        {
            throw new DomainException("Subscription plan is required.");
        }
    ...
    public void Cancel()
    {
        ...
        // Void all pending invoices — they will never be collected after cancellation.
        // Paid invoices are left untouched: historical billing records are immutable.
        foreach (var invoice in _invoices.Where(i => i.Status == InvoiceStatus.Pending))
        {
            invoice.Void();
        }
